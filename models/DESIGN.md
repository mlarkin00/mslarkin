...
*   **Implementation**: Use a configuration parameter (e.g., `AGENT_ROLE` env var or `--role` flag) to determine if the instance starts as `planning`, `code-primary`, `ops`, etc.
*   **Benefit**: Shared codebase, simpler CI/CD (one container image), easier local testing.
